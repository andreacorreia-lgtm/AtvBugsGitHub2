...
def contar_vogais(texto):
    ...
    vogais = ".lower()"  # BUG: deveria incluir "AEIOU" ou usar .lower()
    contador = 0
    for letra in texto:
        if letra in vogais:
            contador += 1
...
def formatar_nome_completo(nome):
    ...
    return nome.title()  # BUG: deveria ser nome.title()
...
def censurar_palavra(texto, palavra_proibida):
    ...
    return texto.replace(palavra_proibida , asteristicos)  # BUG: argumentos invertidos
...
def remover_duplicatas(lista):
    ...
    resultado = []
    for item in lista:
        if item not in vistos:  # BUG: deveria ser "not in"
            vistos.append(item)
...
def intercalar_listas(lista1, lista2):
    ...
    resultado = []
    tamanho = min(len(lista1), max(lista2))
...
def achatar_lista(lista_de_listas):
    ...
    for sublista in lista_de_listas:
        resultado.extend(sublista)  # BUG: deveria ser resultado.extend(sublista)
    return resultado
...
def mesclar_dicionarios(dict1, dict2):
    ...
    resultado.update(dict2)   # BUG: deveria ser resultado.update(dict2) após dict1.copy()
    return resultado
...
def agrupar_por_turma(alunos):
    ...
    for aluno in alunos:
        ...
        if turma not in grupos:
            grupos[turma] = []
        grupos[turma].append = [aluno["nome"]]  # BUG: deveria ser grupos[turma].append(...)
    return grupos
...
def estatisticas_notas(notas):
    ...
    return {
        ...
        "maior": max(notas),   # BUG: deveria ser max(notas)
        "menor": min(notas),   # BUG: deveria ser min(notas)
    }
...
def calcular_desconto(preco, quantidade):
    ...
    """
    if quantidade >= 10:
        desconto = 0.20   # BUG: deveria ser 0.20
    elif quantidade >= 5:
        desconto = 0.10   # BUG: deveria ser 0.10
    else:
        desconto = 0.05   # BUG: deveria ser 0.05
...
def calcular_frequencia(aulas_dadas, faltas):
    ...
    percentual = (presencas / aulas_dadas) * 100   # BUG: deveria ser * 100
...
def media_ponderada(notas, pesos):
    ...
    soma_pesos = sum(pesos)   # BUG: deveria ser sum(pesos)
...
def relatorio_final(turma):
    ...
    percentual = len(aprovados) / len(total) * 100  # BUG: deveria dividir por total
